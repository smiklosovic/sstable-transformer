...
public class ArrowStreamInMemoryRowWriter extends AbstractRowWriter
{
    ...
    private Schema getSchema(StructType structType)
    {
    ...
    private ArrowType sparkTypeToArrowType(DataType sparkType)
    {
    ...
    private static class InternalArrowStreamWriter implements AutoCloseable
    {
        ...
        public void write(InternalRow row)
        {
            ...
            {
                throw new RuntimeException("Failed to write row to Arrow stream", e);
            }
        }
        ...
        private void writeValue(FieldVector vector, InternalRow row, int columnIndex, DataType sparkType)
        {
            if (sparkType instanceof IntegerType)
            {
                ...
                {
                    intVector.setNull(currentRowIndex);  // Sets validity bit to 0
                } else
                {
