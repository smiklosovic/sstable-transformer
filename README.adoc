== SSTable Parquet Transformer

This tool transforms Cassandra's SSTable(s) to Parquet files(s). SSTables can be stored locally
...
When packages are installed, there will be `/usr/local/bin/sstable-parquet-transformer` script which
invokes the tool.

=== Usage

The tool has two subcommands for now, called `sstable2parquet` and `partitions`.

----
$ java -jar target/sstable-parquet-transformer-1.0.0-SNAPSHOT-bundled.jar help
Usage: transformer [-hV] [COMMAND]
Transforms Cassandra SSTable to Parquet file
  -h, --help      Show this help message and exit.
  -V, --version   Print version information and exit.
Commands:
  sstable2parquet  Transform SSTables to Parquet files
...
* `ONE_PARQUET_ALL_SSTABLE`
* `ONE_PARQUET_PER_SSTABLE`

The default is `ONE_PARQUET_PER_SSTABLE` when not specified,
so you will have as many Parquet files as many input SSTables there are.

For compression of Parquet files, there is a support of all compression algorithms
which are supported by Parquet. This option, `--compression` , is just pushed down
to a Parquet writer and data are compressed accordingly.
...
specified by `--max-rows-per-parquet-file` option.
...
direct access to the disk. However, we can use only `ONE_PARQUET_ALL_SSTABLES` for
...
----
java -jar target/sstable-parquet-transformer-1.0.0-SNAPSHOT-bundled.jar sstable2parquet \
  --keyspace=spark_test \
  --table=test3 \
...
----
java -jar target/sstable-parquet-transformer.jar sstable2parquet \
  --create-table-statement='CREATE TABLE spark_test.test3 (id int PRIMARY KEY, col1 int, col2 int)' \
  --strategy=ONE_PARQUET_PER_SSTABLE \
...
----
$ java -jar target/sstable-parquet-transformer-1.0.0-SNAPSHOT-bundled.jar partitions \
    --dc=dc1 \
    --keyspace=spark_test \
...
----
val partitions = CassandraPartitionsResolver.partitions(
  "dc1", "spark_test", "spark-master-1", 9043
).toSeq
...
sc.parallelize(partitions, 6)
  // maps each partition to transformer which will transform data for it to Parquet files
  .map(p => Transformer.runTransformation(builder.partition(p).build()))
  .count()
----
...
----
 $ java -jar target/sstable-parquet-transformer.jar help sstable2parquet
Usage: transformer sstable2parquet [--bloom-filter]
                                   [--compression=<compression>]
                                   [--create-table-statement=<createTableStmt>]
                                   [--keyspace=<keyspace>]
                                   [--max-rows-per-parquet-file=<maxRowsPerFile>
                                   ] --output=<output>
      ...
      --compression=<compression>
                           Use compression for output Parquet files. Can be one
                             of ZSTD, LZ4, BROTLI, LZO, GZIP, SNAPPY,
                             UNCOMPRESSED. Defaults to ZSTD.
      --create-table-statement=<createTableStmt>
      ...
      --max-rows-per-parquet-file=<maxRowsPerFile>
                           Maximal number of rows per Parquet file. Has to be
                             bigger than 0. Defaults to undefined which will
                             put all rows to one Parquet file.
      --output=<output>    Output Parquet file
      --parallelism=<parallelism>
      ...
      --strategy=<transformationStrategy>
                           Whether to convert all SSTables into one Parquet
                             file or there will be one Parquet file per
                             SSTable. Can be one of ONE_PARQUET_PER_SSTABLE,
                             ONE_PARQUET_ALL_SSTABLES. Defaults to
                             ONE_PARQUET_PER_SSTABLE - can not be used when
...
----
$ java -jar target/sstable-parquet-transformer-1.0.0-SNAPSHOT-bundled.jar help partitions
Usage: transformer partitions --dc=<dc> --keyspace=<keyspace>
                              --sidecar=<sidecar> [COMMAND]
      --dc=<dc>
      --keyspace=<keyspace>
      --sidecar=<sidecar>
