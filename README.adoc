...
The core transformer is able to transform SSTable data to Parquet / Avro. If you want to sink transformed data to e.g. ClickHouse, you can do so by leveraging modular architecture of this transformer.

*For the usage of ClickHouse sinks, please consult link:clickhouse/README.adoc[README] in `clickhouse` directory.*
...
----
./core/scripts/build-dtest-jars.sh
# then, for 5.0
...
The commands below are executed in `core` directory.

----
$ java -jar core/target/sstable-transformer-core-2.0.0-bundled.jar help
...
----
java -jar core/target/sstable-transformer-core-2.0.0-bundled.jar transform \
  --keyspace=spark_test \
  --table=test3 \
...
----
$ java -jar core/target/sstable-transformer-core-2.0.0-bundled.jar partitions \
    --dc=dc1 \
    --keyspace=spark_test \
...
----
 $ java -jar core/target/sstable-transformer-core-2.0.0-bundled.jar transform help
...
Transform SSTables to Parquet or Avro files.
      ...
      --cassandra-version=<cassandraVersion>
                           FIVEZERO or FOURZERO, defaults to FIVEZERO
      --compression=<compression>
      ...
      --max-rows-per-file=<maxRowsPerFile>
                           Maximal number of rows per file. Has to be bigger
                             ...
                             rows to one file.
      --output=<output>    Output file or destination, relevant when not using
                             ARROW_STREAM format.
      --output-format=<outputFormat>
      ...
      --sink-config=<sinkConfig>
                           Configuration file for a sink. Each sink is
                             responsible for parsing its content.
...
----
 java -jar core/target/sstable-transformer-core-2.0.0-bundled.jar help partitions
